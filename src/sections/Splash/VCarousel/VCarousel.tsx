...
export interface VCarouselProps extends Omit<HTMLAttributes<HTMLUListElement>, 'children'> {
  items: ReactNode[];
  pause?: boolean;
  duration?: number;
}

function VCarousel({ items, pause = false, duration = 3000, ...props }: VCarouselProps) {
  ...
  const setCurrentDim = useCallback((animate?: boolean) => {
    if (!rootRef.current) return;
    const { offsetWidth, offsetHeight, offsetTop } = itemsRef.current[current];
